 ...
 *
 * @param  string fileName                - initialization file name
 ...
 *
 * @return string - the found configuration value or the specified default value with trailing white space removed;
 *                  an empty string in case of errors (check last_error).
 */
string GetIniStringRaw(string fileName, string section, string key, string defaultValue = "") {
   if (!StringLen(fileName)) return(_EMPTY_STR(catch("GetIniStringRaw(1)  invalid parameter fileName "+ DoubleQuoteStr(fileName), ERR_INVALID_PARAMETER)));
   if (!StringLen(section))  return(_EMPTY_STR(catch("GetIniStringRaw(2)  invalid parameter section "+ DoubleQuoteStr(section), ERR_INVALID_PARAMETER)));
   if (!StringLen(key))      return(_EMPTY_STR(catch("GetIniStringRaw(3)  invalid parameter key "+ DoubleQuoteStr(key), ERR_INVALID_PARAMETER)));

   string buffer[];
   int bufferSize = 64;
   int chars = bufferSize-1;
...
/**
 * Handler for order related errors which occurred in the library's order functions. The execution flags passed to the calling
 * order function determine whether an error is filtered and silently set or causes a terminating ERROR alert. Filtered errors
 * must be handled by a custom error handler.
 *
...
int Order.HandleError(string message, int error, int filter, int oe[], bool refreshPrices = false) {
   ...
   bool singleOE = ArrayDimension(oe)==1;       // whether a single struct or an array of structs ORDER_EXECUTION was passed
   ...
   // in Strategy Tester always add ERS_EXECUTION_STOPPING to the filter
   if (This.IsTesting() && IsStopped())
      filter |= F_ERS_EXECUTION_STOPPING;

   // filter the specified errors but log them
...
int OrderSendEx(string symbol/*=NULL*/, int type, double lots, double price, double slippage, double stopLoss, double takeProfit, string comment, int magicNumber, datetime expires, color markerColor, int oeFlags, int oe[]) {
   ...
   while (true) {
      ...
      if (IsStopped()) return(!Order.HandleError("OrderSendEx(18)  "+ OrderSendEx.ErrorMsg(oe), ERS_EXECUTION_STOPPING, oeFlags, oe));
...
bool OrderModifyEx(int ticket, double openPrice, double stopLoss, double takeProfit, datetime expires, color markerColor, int oeFlags, int oe[]) {
   ...
   if (!isPendingType)
      if (!EQ(openPrice, OrderOpenPrice(), digits))            return(_false(Order.HandleError("OrderModifyEx(11)  cannot modify open price of already open position #"+ ticket, ERR_INVALID_TRADE_PARAMETERS, oeFlags, oe), OrderPop("OrderModifyEx(12)")));
   // stopLoss
   stopLoss = NormalizeDouble(stopLoss, digits);
   ...
   if (EQ(openPrice, prevOpenPrice, digits)) /*&&*/ if (EQ(stopLoss, prevStopLoss, digits)) /*&&*/ if (EQ(takeProfit, prevTakeProfit, digits)) {
      warn("OrderModifyEx(24)  nothing to modify for ticket #"+ ticket);
   ...
   while (true) {
      ...
      if (IsTradeContextBusy()) {
         if (__LOG()) log("OrderModifyEx(29)  trade context busy, retrying...");
         Sleep(300);
         continue;
      }
      ...
      switch (error) {
         case ERR_TRADE_CONTEXT_BUSY:
            if (__LOG()) log("OrderModifyEx(35)  trade context busy, retrying...");
            Sleep(300);
            continue;
         ...
         case ERR_TRADE_TIMEOUT:
            ...
            warn("OrderModifyEx(36)  "+ Order.TempErrorMsg(oe, tempErrors), error);
            continue;

         // map terminal generated errors
         case ERR_INVALID_TICKET:                // unknown ticket or not open pending order anymore (client-side)
            if (__LOG()) log("OrderModifyEx(37)  translating returned ERR_INVALID_TICKET => ERR_INVALID_TRADE_PARAMETERS");
            error = ERR_INVALID_TRADE_PARAMETERS;
            break;

         case NO_ERROR:
            if (__LOG()) log("OrderModifyEx(38)  no error returned => ERR_RUNTIME_ERROR");
 ...
 *                                            each other (anymore)
 *
 *        (2) On partial close the total values for swap, commission and profit may be divided by the trade server over both
   ...
   */
   ...
   while (true) {
      ...
      if (success) {
         ...
         if (__LOG()) log("OrderCloseByEx(26)  "+ OrderCloseByEx.SuccessMsg(first, second, largerType, oe));
      ...
      switch (error) {
         case ERR_TRADE_CONTEXT_BUSY:
            if (__LOG()) log("OrderCloseByEx(28)  trade context busy, retrying...");
            Sleep(300);
            continue;
         ...
         case ERR_TRADE_TIMEOUT:
            tempErrors++;
            if (tempErrors > 5) break;
            warn("OrderCloseByEx(29)  "+ Order.TempErrorMsg(oe, tempErrors), error);
            continue;
...
int OrdersHedge(int tickets[], double slippage, int oeFlags, int oes[][]) {
   ...
   if (EQ(totalLots, 0)) {
      ...
      if (__LOG()) log("OrdersHedge(13)  "+ sizeOfTickets +" "+ symbol +" positions "+ TicketsToStr.Lots(tickets, NULL) +" are already flat");
   ...
   */
   ...
   while (true) {
      ...
      if (success) {
         ...
         if (__LOG()) log(StringConcatenate("OrderDeleteEx(13)  ", OrderDeleteEx.SuccessMsg(oe)));
         if (!IsTesting())
      ...
      switch (error) {
         case ERR_TRADE_CONTEXT_BUSY:
            if (__LOG()) log("OrderDeleteEx(15)  trade context busy, retrying...");
            Sleep(300);
            continue;
         ...
         // map terminal generated errors
         case ERR_INVALID_TICKET:                // unknown ticket or not open pending order anymore (client-side)
            if (__LOG()) log("OrderDeleteEx(17)  translating returned ERR_INVALID_TICKET => ERR_INVALID_TRADE_PARAMETERS");
            error = ERR_INVALID_TRADE_PARAMETERS;
            break;
         case ERR_OFF_QUOTES:                    // order is locked and in processing queue (server-side)
            if (__LOG()) log("OrderDeleteEx(18)  translating returned ERR_OFF_QUOTES => ERR_INVALID_TRADE_PARAMETERS");
            error = ERR_INVALID_TRADE_PARAMETERS;
            break;
